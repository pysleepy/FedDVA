...
from models.DualEncoders_celeba import DualEncoder, loss_dkl, loss_reg_c
...
class FedClient:
    ...
    def fit(self, device, cur_round, tr_loader, epoch_encoder, epoch_decoder, lr, n_resamples):
        ...
        for ep in range(epoch_encoder):
            logging.info("Round: {:d}, Client: {:d}, Epoch Enc: {:d}".format(cur_round, self.client_id, ep))
            print("Round: {:d}, Client: {:d}, Epoch Enc: {:d}".format(cur_round, self.client_id, ep))
            epoch_dec = []
            epoch_dkl_z = []
            epoch_dkl_c = []
            epoch_constr_c = []
            for b_id, data in enumerate(tr_loader):
                ...
                loss = self.lbd_dec * loss_dec + self.lbd_z * loss_dkl_z + self.lbd_c * loss_dkl_c \

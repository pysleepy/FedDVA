...
class FedManager(BaseManager):
    pass

fed_manager = FedManager()
fed_manager.register("Client", DualEncodersDigits)
fed_manager.start()
...
# init models
global_model = DualEncodersDigits(-1, args.shared_modules.union('decoder_z', 'decoder_c')
...
client_models = [fed_manager.Client(c_id, args.shared_modules.union('decoder_z', 'decoder_c')
                                    , args.optimizer_func, args.criterion
                                    , args.d_z, args.d_c, args.xi
                                    , args.lbd_dec_z, args.lbd_dec_c, args.lbd_z, args.lbd_c, args.lbd_cc
                                    , args.n_resamples, 1)
...
for r in range(args.n_rounds):
    print("round: " + str(r))
    clts_p = [mp.Process(target=client_model.fit, args=(args.device
                                                        , r
                                                        , tr_loader
                                                        , args.epoch_encoder
                                                        , args.epoch_decoder
...
# outputs = client_models[0].evaluate(0, ts_loaders[0], args.criterion)

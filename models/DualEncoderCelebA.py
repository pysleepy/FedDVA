...
class BackboneCelebA(Backbone):
    def __init__(self, in_channel, hidden_dims):
        super(BackboneCelebA, self).__init__()
...
class DualEncoderCelebA(DualEncoder):
    ...
    def __init__(self, d_z, d_c):
        ...
        self.backbone_z = BackboneCelebA(self.in_channel, self.hidden_dims)
        ...
        self.backbone_c = BackboneCelebA(self.in_channel + 1, self.hidden_dims)
        self.encoder_c = Encoder(self.hidden_dims[-1], self.d_c)

        self.decoder = DecoderCelebA(self.d_z + self.d_c, self.hidden_dims, self.in_channel)

...
def aggregate(model_target, model_list, n_sample_list):
    ...
    for model_source, n_sample in zip(model_list, n_sample_list):
        ...
        for md_target, md_source in zip(model_target.model.named_parameters(), model_source.model.named_parameters()):
            if md_source[0].split(".")[0] in model_source.shared_list:
